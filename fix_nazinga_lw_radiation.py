"""
Fix swapped longwave radiation (LW_in and LW_out) for Nazinga from 2018 onwards.

This script corrects the longwave radiation data in the Parquet file by swapping
IR_in_Avg and IR_out_Avg for data from 2018-01-01 onwards.
"""
...
def main():
    ...
    print(f"   IR_in_Avg:  mean={df_after['IR_in_Avg'].mean():.2f}, min={df_after['IR_in_Avg'].min():.2f}, max={df_after['IR_in_Avg'].max():.2f}")
    print(f"   IR_out_Avg: mean={df_after['IR_out_Avg'].mean():.2f}, min={df_after['IR_out_Avg'].min():.2f}, max={df_after['IR_out_Avg'].max():.2f}")
    
    # Swap IR_in_Avg and IR_out_Avg for data >= 2018-01-01
    print(f"\n5. Swapping IR_in_Avg and IR_out_Avg for data >= {FIX_DATE}...")
    
    # Create copies of the columns
    ir_in_original = df['IR_in_Avg'].copy()
    ir_out_original = df['IR_out_Avg'].copy()
    
    # Swap only for data >= fix_date
    df.loc[mask_after_fix, 'IR_in_Avg'] = ir_out_original[mask_after_fix]
    df.loc[mask_after_fix, 'IR_out_Avg'] = ir_in_original[mask_after_fix]
    
    print(f"   ✓ Swapped {records_to_fix} records")
    ...
    # Verify the swap (IR_in should now have values that were in IR_out, and vice versa)
    print(f"\n7. Verifying swap...")
    ir_in_swapped_correctly = (df.loc[mask_after_fix, 'IR_in_Avg'] == ir_out_original[mask_after_fix]).all()
    ir_out_swapped_correctly = (df.loc[mask_after_fix, 'IR_out_Avg'] == ir_in_original[mask_after_fix]).all()
    
    if ir_in_swapped_correctly and ir_out_swapped_correctly:
        print(f"   ✓ Swap verified successfully")
    else:
        print(f"   ⚠️  Warning: Swap verification failed. Please check manually.")
